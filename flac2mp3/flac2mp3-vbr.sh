...
#
# flac2mp3-vbr.sh - Linux script to convert FLAC audio files to MP3 files using VBR.
#
# Usage:
#
#   Background: ./flac2mp3-vbr.sh </dev/null >flac2mp3-vbr.log 2>&1 &
#   Foreground: ./flac2mp3-vbr.sh 2>&1 | tee flac2mp3-vbr.log
#
...
#
# Set the SOURCEDIR, TARGETDIR, COVERART, VBRATE and CORES per your requirements.
#

SOURCEDIR="PATH_TO_FLAC/flac"
TARGETDIR="PATH_TO_MP3/mp3"
COVERART="cover.jpg"
VBRATE="4" # 0-9: 0 is highest quality.
CORES=4
        ...
        do
          ...
          if [ -f "${SOURCEART}" ]; then
            ...
            ffmpeg -nostdin -loglevel error -i "${SOURCE[$INDEX]}" \
              ...
              -map_metadata 0 -codec:a libmp3lame -q:a ${VBRATE} "${TARGET[$INDEX]}" &
            PID[INDEX]=$!
          else
